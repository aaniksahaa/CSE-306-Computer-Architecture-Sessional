    ...
    for i in range(len(statements)):
        ...
        if(len(instr_name) > 0):
            if(len(pending_label) > 0):
                label_to_address_dict[pending_label] = len(instructions)
                pending_label = ''
            instr = Instruction(instr_name, args, len(instructions), i+1)
...
def convert_single_instruction(instr):
    ...
    if(instr_name in ['addi','subi','andi','ori']):
        if(len(args) != 3):
            error_msg = 'Wrong number of arguments'
            return error_msg, machine_code
        else:
...
def parse_instructions(instructions):
    ...
    while(at < len(instructions)):
        ...
        if(instr.instr_name == 'push'):
            ...
            if(len(error_msg) > 0):
                print('Error at line ' + str(instr.line_no) + ': ' + error_msg)
                error_occurred = True
            else:
...
def convert_push_memory(instr):
    if(instr.instr_name != 'push'):
        print("\n\nUNEXPECTED ERROR, not a push instruction\n\n")
    dummy_1 = Instruction('lw',['$hd',instr.args[0]],instr.instr_no,instr.line_no)
    dummy_2 = Instruction('sw',['$hd','0($sp)'],instr.instr_no,instr.line_no)
    dummy_3 = Instruction('subi',['$sp','$sp','1'],instr.instr_no,instr.line_no)
    return dummy_1, dummy_2, dummy_3



instr = Instruction('push', ['5($t0)'],-1,-1)
